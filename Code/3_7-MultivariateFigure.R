# source('./Code/3_7-MultivariateFigure.R')
source('./Code/0-Param.R')
load('./Data/Multivariate1P.RData')
load('./Data/MultivariateAdditivity1P.RData')
load('./Data/MultivariateAddPosition1P.RData')
load('./Data/MultivariateAddPathway1P.RData')
load('./Data/MultivariatePathway1P.RData')
load('./Data/MultivariatePosition1P.RData')

# Parameters
# Mean abundances < | > 0.1
id1 <- max(which(int$Mean < -.01))
id2 <- min(which(int$Mean > .01))
id0 <- range(which(int$Mean == 0))

# Additivity
idA1 <- max(which(intAdd$Mean <= -.0005))
idA2 <- min(which(intAdd$Mean >= .0005))


# ------------------------------------------------------------------------------
#                                 FIGURE 2
# ------------------------------------------------------------------------------
png('./Figures/multivariateDisturbance1P.png', width = 1200, height = 780, res = 200, pointsize = 6)
...
# ------------------------------------------------------------------------------
par(mar = c(2,6,2,1))
plot0(x = c(0,nrow(int)), y = c(-.2, .2))
axis(2, at = seq(-.2,.2,by = .05), las = 2)
...
polygon(x = c(1:nrow(int), nrow(int):1), y = c(int$CIp, rev(int$CIm)), col = '#23349922', border = 'transparent')
...
text(x = u[1]+50, y = u[4]-.015, labels = 'Weak entry points', adj = c(0,.5), font = 2)
text(x = u[2]-50, y = u[4]-.015, labels = 'Weak entry points', adj = c(1,.5), font = 2)
text(x = mean(id0), y = u[4]-.015, labels = 'Biotic buffers', adj = c(.5,.5), font = 2)
...
# ------------------------------------------------------------------------------
par(mar = c(2,6,2,1))
plot0(x = c(1,nrow(intAdd)), y = c(-.005, .005))
axis(2, at = seq(-.005, .005, by = .0025), las = 2)
abline(h = 0, lty = 2, col = '#00000055')
...
par(mar = c(2,6,1,1))
plot0(x = c(1,nrow(spProf)), y = c(-.075, .075))
axis(2, at = round(seq(-.075,.075,by=.025),2), las = 2)
...
for(i in 1:nrow(spProf)) {
  plotMotifs(motif = motif[i], position = sp[i],
             path = '', x = i, y = -.0675, scalingY = .005, scalingX = .45,
...
par(mar = c(2,6,1,1))
plot0(x = c(1,nrow(spProfAdd)), y = c(-.0008, .0008))
axis(2, at = seq(-.0008,.0008,by=.0004), las = 2)
...
for(i in 1:nrow(spProfAdd)) {
  plotMotifs(motif = motif[i], position = sp[i],
             path = '', x = i, y = -.0007, scalingY = .000075, scalingX = .45,
             add = T, cex = 1, lwd = 1.15, lwd2 = 1.25)
}
...
par(mar = c(2,6,.5,1))
plot0(x = c(1,nrow(pProf)), y = c(-.08, .08))
axis(2, at = seq(-.08,.08,by=.04), las = 2)
...
par(mar = c(2,6,.5,1))
plot0(x = c(1,nrow(pProfAdd)), y = c(-.0008, .0008))
axis(2, at = seq(-.0008,.0008,by=.0004), las = 2)
