...
for(k in pos) {
  ...
  # Thresholds
  # Mean abundances < | > 0.1
  id1 <- which(round(dat$Mean,6) < -.01) %>% { if(length(.) > 0) max(.) else NULL }
  id2 <- which(round(dat$Mean,6) > .01) %>% { if(length(.) > 0) min(.) else NULL }
  id0 <- which(round(dat$Mean,6) == 0) %>% { if(length(.) > 0) range(.) else NULL }

  # Plot
  par(mar = c(0,3,0,0))
  plot0(x = c(1,nrow(dat)), y = c(-.12, .12))
  u <- par("usr")
  axis(2, las = 2)
...
for(k in pos) {
  ...
  # Plot
  par(mar = c(0,3,0,0))
  plot0(x = c(1,nrow(dat)), y = c(-.005, .005))
  u <- par("usr")
  axis(2, las = 2)
  abline(h = 0, lty = 2, col = '#00000088')
  polygon(x = c(1:nrow(dat), nrow(dat):1), y = c(dat$CIm, rev(dat$CIp)), col = '#23349955', border = 'transparent')
  ...
  yG <- .0012
  if (!is.null(idA1)) {
    lines(x = rep(idA1+.5,2), y = c(u[3]+yG, u[4]-yG), lty = 4, col = '#668aeb')
    polygon(x = c(u[1],idA1+.5, idA1+.5,u[1]), y = c(u[3]+yG, u[3]+yG, u[4]-yG,u[4]-yG), border = 'transparent', col = '#668aeb22')
  }
  if (!is.null(idA2)) {
