...
png('./Figures/TL-OM-Realised.png', width = 800, height = 800,
    res = 200, pointsize = 6)
...
# Add sensitivity and amplification to igraph objects
maxSens <- max(abs(
c(impact[[1]]$Sensitivity, impact[[2]]$Sensitivity, impact[[3]]$Sensitivity)
))
maxAmp <- max(abs(unlist(spAmplification)))
...
for(i in 1:3) {
  # Sensitivity data
  x <- impact[[i]]
  uid <- match(vertex.attributes(dat[[i]])$name, x$species)
  x <- x[uid, ]

  # Add to igraph object
  dat[[i]] <- dat[[i]] %>% set_vertex_attr("score", value = abs(x$Score))
  ...
  uid0 <- x$Score == 0
  uidP <- x$Score > 0
  cols[uidN] <- pal2(101)[y[uidN] + 1]
  cols[uid0] <- '#ffffff'
  cols[uidP] <- pal1(101)[y[uidP] + 1]
  dat[[i]] <- dat[[i]] %>% set_vertex_attr("color", value = cols)
}

# plot igraphs
png('./Figures/speciesImpact.png', width = 1800, height = 700,
    res = 200, pointsize = 6)
...
for(i in 1:3) {
  ...
  lay[,2] <- x$TL
  plot(dat[[i]],
       vertex.size = vertex_attr(dat[[i]], 'score')*2,
...
# Trophic level and omnivory
png('./Figures/TL-OM-Impact.png', width = 800, height = 800,
    res = 200, pointsize = 6)
par(mfrow = c(3,2), family = 'serif', mar = c(4,4,.5,.5))
for(i in 1:3) {
  x <- NetIndices::TrophInd(mat[[i]])
  uid <- match(impact[[i]]$species, rownames(x))
  x <- x[uid, ]
  sens = impact[[i]]$Score
  y <- cbind(x, sens)
