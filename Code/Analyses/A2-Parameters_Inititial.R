...
df <- matrix(ncol = nVar+nFix, nrow = iter, dimnames = list(c(), c(nmFixed, nmVar)))
...
df <- matrix(fixed, ncol = 6, nrow = 2, dimnames = list(c(), c(nmFixed)), byrow = T)

# Export parameters
diParam <- as.data.frame(df)
diParam <- diParam[1,]
