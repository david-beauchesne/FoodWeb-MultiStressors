# source('./Code/4-UnivariateFigure.R')
...
# Parameters
# Mean abundances < | > 0.1
id1 <- max(which(round(uni$Mean,2) <= -.1))
id2 <- min(which(round(uni$Mean,2) >= .1))
id0 <- range(which(round(uni$Mean,2) == 0))

# ------------------------------------------------------------------------------
#                                 FIGURE 1
# ------------------------------------------------------------------------------
png('./Figures/univariateDisturbance.png', width = 1400, height = 1250, res = 200, pointsize = 6)
par(mfrow = c(2,1))

# Plot 1
par(mar = c(2,5,.5,.5), family = 'serif')
plot0(x = c(1,nrow(uni)), y = c(-.75, .75))
axis(2, at = seq(-.75,.75,by=.25), las = 2)
...
text(x = u[2]-5, y = u[3]+.35, labels = TeX('Mortality / growth rate ($r$, $m$)'), adj = c(1, .5))
text(x = u[2]-5, y = u[3]+.30, labels = TeX('Attack rate / predator avoidance ($\\beta$, $\\gamma$, $\\delta$)'), adj = c(1, .5))
text(x = u[2]-5, y = u[3]+.25, labels = TeX('Conversion rate / palatability ($\\mu$, $\\nu$, $\\omega$)'), adj = c(1, .5))
points(x = u[2]-3.5, y = u[3]+.3525, col = '#db5656', bg = '#00000099', pch = 21, lwd = 2, cex = 1)
lines(x = c(u[2]-3, u[2]-4), y = rep(u[3]+.3025, 2), lwd = 2, col = '#db5656')
lines(x = c(u[2]-3, u[2]-4), y = rep(u[3]+.2525, 2), lwd = 2, col = '#53998e')
...
text(x = mean(c(id1,u[1])), y = u[4]-.1, labels = 'Weak entry points', adj = c(.5,.5), font = 2)
text(x = mean(c(id2,u[2])), y = u[4]-.1, labels = 'Weak entry points', adj = c(.5,.5), font = 2)
...
for(i in seq(1,nrow(uni), by = 2)) {
  plotMotifs(motif = uni$motif[i], position = uni$species[i],
             path = uni$parameters[i], x = i, y = -.75, scalingY = .025,
             add = T, cex = .6, lwd = 1, lwd2 = 1.1)
}

for(i in seq(2,nrow(uni), by = 2)) {
  plotMotifs(motif = uni$motif[i], position = uni$species[i],
             path = uni$parameters[i], x = i, y = -.65, scalingY = .025,
             add = T, cex = .6, lwd = 1, lwd2 = 1.1)
}
...
for(i in 1:nrow(spProf)) {
  plotMotifs(motif = motif[i], position = sp[i],
             path = '', x = i, y = -.475, scalingY = .025, scalingX = .35,
